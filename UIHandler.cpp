...
void DrawUI(const GLMaterial& material, const glm::vec2& position, const glm::vec2& scale, int priority)
{
	static GLMesh uiplane(
		...
		{ 0, 1, 2, 0, 2, 3 });
	uiplane.Draw(material, glm::vec3(position.x * 2 - 1, position.y * 2 - 1, 1-(float)priority/100), glm::quat(1, 0, 0, 0), glm::vec3(scale.x, scale.y, 1));
}	
...
void DrawUI(const GLMaterial& material, std::vector<GLfloat> vertices, int priority)
{
	static GLMesh uiplane(vertices, { 0, 1, 2, 0, 2, 3 });
	uiplane.SetData(vertices);
	uiplane.Draw(material, glm::vec3(0, 0, 1 - (float)priority / 100), glm::quat(1, 0, 0, 0), glm::vec3(1, 1, 1));
}
